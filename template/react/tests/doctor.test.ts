  ...
  const env = Object.fromEntries(
    Object.entries(process.env).filter(
      ([key]) =>
        ...
        key !== "VITE_FILECOIN_NETWORK" &&
        !/^VITE_.*(?:PRIVATE|SECRET|MNEMONIC|SEED)/i.test(key),
    ),
  );
...
test("doctor rejects unsupported networks", () => {
  const result = runDoctor({ VITE_FILECOIN_NETWORK: "unsupported" });
  assert.equal(result.status, 1);
  assert.match(result.stdout, /Unsupported VITE_FILECOIN_NETWORK=unsupported/);
});

test("doctor rejects private keys exposed through public environment variables", () => {
  const result = runDoctor({ VITE_PRIVATE_KEY: "unsafe" });
  assert.equal(result.status, 1);
  assert.match(result.stdout, /Remove public secret variables: VITE_PRIVATE_KEY/);
});
