...
const requiredFiles = ["package.json", "src/App.tsx", "src/lib/foc/adapter.ts", ".env.example"];
...
try {
  ...
  const missingDependencies = ["@filoz/synapse-sdk", "viem", "react"].filter((name) => !dependencies[name]);
  if (missingDependencies.length === 0) report("PASS", "FOC dependencies", "Synapse SDK, viem, and React are declared.");
...
const network = env.VITE_FILECOIN_NETWORK || "__FOC_NETWORK__";
...
else report("FAIL", "Network", `Unsupported VITE_FILECOIN_NETWORK=${network}`);

const publicSecretKeys = Object.keys(env).filter((key) =>
  /^VITE_.*(?:PRIVATE(?:_KEY)?|SECRET(?:_KEY)?|MNEMONIC|SEED(?:_PHRASE)?)/i.test(key),
);
